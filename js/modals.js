...
function openReturnToQMOWReasonModal({ task, fromStatus, sourceLabel, onSubmit, onCancel }) {
  ...
  const submitBtn = document.getElementById("returnToQMOWSubmitBtn");
  const cancelBtn = document.getElementById("returnToQMOWCancelBtn");

  if (!modal || !reasonEl || !contextEl || !submitBtn || !cancelBtn) return;
  ...
  // Re-grab after replace (if cleanup runs before binding again)
  const bindButtons = () => {
    ...
    submit.addEventListener("click", () => {
      ...
      if (!text) {
        alert("Feedback is required to return a task to QMOW.");
        return;
      }
  ...
  // If already bound from a previous open, hard reset binding by cleanup then bind
  cleanup();
  bindButtons();
}
...
function openTaskDetailModal(e) {
  ...
  if (detailModal) detailModal.classList.add("is-visible");
...
function renderTaskHistory(task) {
  ...
  sortedHistory.forEach((item) => {
    if (template) {
      ...
      div.innerHTML = `${formatTimestampForHistory(item.timestamp)} ${item.action}`;
...
function handleStatusChange(e) {
  ...
  // ✅ NEW: only moves TO QMOW require mandatory feedback prompt
  if (newStatus === "qmow" && oldStatus !== "qmow") {
    openReturnToQMOWReasonModal({
      ...
      onSubmit: (feedbackText) => {
        const ok = applyStatusChange(task, "qmow", {
          actor: "User A",
...
function toggleMetadataEdit(e) {
  ...
  if (isEditing) {
    ...
    if (changesMade) {
      if (!Array.isArray(task.history)) task.history = [];
      task.history.push({ timestamp: new Date().toISOString(), action: historyAction });
    }
    ...
    unitSpan.innerHTML = `<input id="input-unit-name" type="text" value="${task.unitName}" />`;
    opSpan.innerHTML = `<input id="input-op-name" type="text" value="${task.operationName}" />`;
    dtgSpan.innerHTML = `<input id="input-dtg" type="text" value="${task.dtg}" />`;
  }
}
...
function handleNewTaskSubmit(e) {
  ...
  if (!dtgDate) {
    if (validationMsg) {
      validationMsg.textContent = "Invalid DTG format. Must be DDHHMMZMMMYY (e.g., 101330Z DEC 25)";
